'''
- mysqldb dicr curser quicker than pandas_read_sql
- test post-peak death trace
...
from bokeh.models import ColumnDataSource, Select, CDSView, GroupFilter, BooleanFilter, DatetimeTickFormatter, LabelSet, Label, Title
...
def db2(sql):
    connection = MySQLdb.connect(host='192.168.1.173', port=3306, user='pi',
                                 password='7914920', db='Health')
    df = read_sql(sql, connection) #, parse_dates=d, index_col='ts') # LIMIT 28800"
    connection.close()
    return df

st=timer() # setup time timer

#df_pays = db2("SELECT * FROM countries")
#ghes = db2("SELECT ghe, C FROM ghe")
#dfall = db2("SELECT * FROM C19daily")
...
print(f'Setup time : {fi-st}') # mysqldb ~ 0.45 BUT pandas_read_sql ~0.52
...
dfall['dfm_dmax'] = dfall.dmax / gbdm.transform('max')
...
plot = figure(plot_height=400, plot_width=640, title='Top 5 causes of death vs C19', tools="save", toolbar_location="right", sizing_mode='scale_width')
...
plot.add_layout(Title(text="Sources : ECDC Europa. WHO Global", text_font_style="italic"), 'below')
...
# Set up plot1 cases
TOOLTIPS1 = [("cases", "$y{0.}"), ("% change", "@dpc_cases{0.0}")]
plot1 = figure(plot_height=200, x_axis_type="datetime", y_axis_label='cases', tooltips=TOOLTIPS1,
               title = 'Cases, Deaths with Daily changes vs weekly mean', tools="save", toolbar_location="above")
...
TOOLTIPS2 = [("deaths", "$y{0.}"), ("% change", "@dpc_deaths{0.0}")]
plot2 = figure(plot_height=200, x_axis_type="datetime", y_axis_label='deaths', tools="save", tooltips=TOOLTIPS2, toolbar_location="above")
...
plot3 = figure(plot_height=400, plot_width = 600, title = 'Country vs Region comparison of post peak performance', x_axis_type='linear',
               x_range = (-1,postpeakd), x_axis_label='days since peak of cases',
               y_axis_label='Post peak ratio of cases (Peak = 1) and death ', y_range = (0, 1.1),
               tooltips=TOOLTIPS, tools="reset, save,wheel_zoom,box_zoom, pan", toolbar_location="right")
plot3.line(x='id', y='dfm_cmax', source=source1, view=view3, legend_label='regional cases')
plot3.line(x='id', y='dfm_cmax', source=source1, view=view4, color = 'red', line_width = 3.0, legend_label='country cases') #,  color = Category20)
plot3.line(x='id', y='dfm_dmax', source=source1, view=view4, color = 'black', line_width = 3.0, legend_label='country deaths') #,  color = Category20)
source3.data = gblast.loc['Western Europe']

label = LabelSet(x='id', y='dfm_cmax', text='geoId', text_font_size='0.8em', x_offset=1, y_offset=-1, source=source3)
pays = Select(title="Select Country ", value="Switzerland", options = sorted(df_pays['Name'].to_list()))
...
#       print(f' s {s} a {a}')
    ...
    def selectdeath(c, ag, s):
        # mortality rate in selected group
        g = (10, 600, 1510)
        print(f'c {c} s{s} a{ag}')
        d = pd.DataFrame(db(f"SELECT {c} FROM death WHERE ghe IN {g} AND sex ='{s}' AND age = '{ag}'"))
        td = d[c].sum()  # tot deaths
        # pop = pd.DataFrame(db(f"SELECT {c} FROM pop WHERE sex = '{s}' AND age = '{a}'")) # pop of sex, age, country
        p = db(f"SELECT {c} FROM pop WHERE sex = '{s}' AND age ='{ag}'")  # tuples of 6 dicts per age group { CHE : pop}
        print(f' pop {p}')
        pop = sum(i[c] for i in p)
        dY = 1  # DoY / 365
        return (dY * 1000 * td / pop)
    ...
    sd = selectdeath(p,ag,s)

    source.data = dict(y=y, x=x, y0 = y0, text=labs, color=Spectral6)
    textbox.text=(f'Annual Mortality per 1k pop')
    textbox1.text = (f'All causes + male/female =  {z[0]:.2f}')
    textbox2.text = (f'Within this selected group = {sd:.2f}')
