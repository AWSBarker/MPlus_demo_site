...
# setup lookup dicts
#rdict = dict(zip(df_pays['Name'], df_pays['Region']))
...
def update_data(attr, old, new):
    ...
    #ps = pays.value
    reg = df_pays[df_pays['Name'] == pays.value].Region.iloc[0] #rdict[pays.value]  # Select(title="Region", value="Vd", options = ddd['Switzerland'])
    pinreg = df_pays.Code3[df_pays['Region'] == reg].to_list() # country list in region

    xx = df_pays[df_pays['Name'] == pays.value].Code.iloc[0]
    print(f"view1 update {xx}")
...
layout2 = column(inputs2, plot1, plot2) #, name='g1')
