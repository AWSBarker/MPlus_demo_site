# try get local IP into initial map from django request saved in data/ip.log
...
# produce cities by selected country each selection. allow code or name selection i.e. dict using name and code keys
    def __init__(self, c, place='', latlon=()):
        ...
        self.aplace = place if place !='' else next(iter(self.locations)) #3 not exact coords ipinfo
...
ip = load_data(join(dirname(__file__), 'data', 'ip.log'))
if ip != 'None':
    content = geocoder.ip(ip).json
    city = content['city']
    country = content['country']
    lat = content['lat'] # need floats
    lon = content['lng']
...
curdoc().session_context.token_payload # returned from request_handler.py