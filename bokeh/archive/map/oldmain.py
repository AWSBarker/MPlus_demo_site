# create class for city by country to avoid public using mysql. single updater
...
# produce cities by selected country each selection. allow code or name selection i.e. dict using name and code keys
    def __init__(self, c):
        ...
        self.aplace = next(iter(self.locations))
        self.acoord = self.locations[self.aplace]
...
def init_plot(l):
    ...
    p = figure(plot_width=800, x_range=a.get_xrange(), y_range=a.get_yrange(), x_axis_type="mercator", y_axis_type="mercator")
...
tile_provider = get_provider(Vendors.STAMEN_TERRAIN) # OSM

p = init_plot(cc.locations[location.value])

controls = row(country, location)
