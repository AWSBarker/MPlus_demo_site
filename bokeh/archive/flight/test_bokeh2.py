...
location = 'HOM'
TOOLTIPS = [("Origin", "@orig"), ("Alt m", "@alt{0.0a}"),
            ("Speed km/h", "@vel{0}"), ("Dir", "@head"), ("Climb m/s", "@vr")]

while True:
    time.sleep(5)
    a = Update(location)
    tile_provider = get_provider(Vendors.CARTODBPOSITRON)  # STAMEN_TERRAIN)
    source = ColumnDataSource(data=dict(lon=a.sslon,
                                        lat=a.sslat,
                                        cs=a.ss,
                                        orig=a.ssorig,
                                        alt=a.ssalt,
                                        ...
                                        vr=a.ssvr,
                                        head=a.ssh))
    p = figure(x_range=a.get_xrange(), y_range=a.get_yrange(),
    ...
    output_file("/mnt/RPi3-data/RPi1-Pictures/flight.html")    # https://awsbarker.ddns.net/test/
    save(p)
