# png static served in eliot/static by bokeh
# add tab for ECG
...
class TempFileURL:
    '''
    - create tf URL of PDF file from input using metadata_correlationID as UUID
    - input is a PDF page 1 of 2 from PIL image
    - output is URL with /tmp/PM100_uuid_ts/random.pdf
    - URL needs folder/file format valid until closure
    '''
    static_folder = 'eliot/static'

    def __init__(self, im, cid, ts):
        self.cid = cid
        self.ts = ts
...
measurements_ecgSamples_samples from eliot2_bk WHERE metadata_measurementType = 'ECG'"""

cols = ['IMEI', 'model', 'tz', 'ts', 'type', 'ucid', 'min', 'max', 'rate', 'rateunit', 'factor', 'factorunit', 'samples']
dfecg = pd.DataFrame([i for i in db_get(sql)],columns=cols)
dfecg['format'] = 'pdf'
dfecg.loc[dfecg.samples.str.startswith('[{'), 'format'] ='raw'
dfecg['aurl'] = dfecg[dfecg['format']=='pdf'].apply(lambda  x: TempFileURL(x.samples,x.ts, x.ucid).png, axis=1)
...
pECG = figure(title="PM100 reports (eliot2_bk for tests)", plot_width= 650, plot_height=1200, y_range=(-1,1), x_range=(-1,1))
pECG.image_url('aurl', x=-1, y=1, source=source6, view=viewIMEI)
...
layout5 = column(data_tablePDF, pECG)
