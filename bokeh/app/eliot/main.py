# lightsail version - need test BS2001 in awsb.ddns
# png static served in /var/www/html/awsb.ddns.net/static by bokeh
# add tab for ECG
...
from bokeh.models import CDSView, GroupFilter, BooleanFilter, ColumnDataSource, Select,  \
    BoxAnnotation, DatetimeTickFormatter, Label,  Range1d, LinearAxis, Slider, HoverTool, \
    DataTable, TableColumn, DateFormatter
...
def db_get(sql): # return LoT ot ToT
    try:
        return DBconx('T').query(sql)
    except:
        return [(makecolour(), 'M', 60, 183),] #colour, sex, age, height

def db_post(data, sql): # data = LoT
    print(f'posting {data}')
    DBconx('T').writemany(sql, data)
...
def makecolour(): # return single random coour TODO for BC800 only else black
    return ["#"+''.join([random.choice('0123456789ABCDEF') for j in range(6)])]

# get redis data
r = redis.Redis()
dfbp = pickle.loads(r.get("eliot_dfbp"))
dfall = pickle.loads(r.get("eliot_dfall"))
dfbc = pickle.loads(r.get("eliot_dfbc"))
dfgtel = pickle.loads(r.get("eliot_dfgtel"))
dfgw = pickle.loads(r.get("eliot_dfgw"))
dfecg = pickle.loads(r.get("eliot_dfecg"))

# setup devices
dfecg_pdf = dfecg[dfecg['format'] == 'pdf']

bp_devices = sorted(dfbp['device_IMEI'].unique().tolist())
bc_devices = sorted(dfbc['device_IMEI'].unique().tolist())
gtel_devices = sorted(dfgtel['device_IMEI'].unique().tolist())
gw_devices = sorted(dfgw['device_IMEI'].unique().tolist())
ecg_devices = sorted(dfecg_pdf.IMEI.unique().tolist())
...
ecg_initvalue = (oneMonBack, dfecg.index.max())
...
#ecgbools = (dfecg.index >= dfecg.index.min()) & (dfecg.index <= dfecg.index.max()) # truncates min , max
...
source6 =ColumnDataSource(dfecg_pdf)
viewIMEI = CDSView(source=source6, filters=[GroupFilter(column_name='IMEI', group=ecg_devices[0])])
...
weights = figure(plot_height=350, plot_width= 640, x_axis_type="datetime", y_axis_label='Kg ', tools=TOOLS, toolbar_location = 'above', title = '3. Select your weight measurements BC 800 only')
weights.background_fill_color = "grey"
weights.background_fill_alpha = 0.3
...
weights.xaxis.formatter=DatetimeTickFormatter(days=["%d.%m.%y"])

# create another new plot, add a renderer that uses the view of the data source
bodycomp = figure(plot_height=350, plot_width= 640, y_range=(0,130), y_axis_label='Body Composition estimate', toolbar_location = None, title='5. Body composition for selected measurements BC800 only') #
...
#ecg plot
columns = [
        TableColumn(field="IMEI", title="IMEI"),
        TableColumn(field="ts", title="ts", formatter=DateFormatter()),
        TableColumn(field ='aurl', title="URL")
    ]
data_tablePDF = DataTable(source=source6, view=viewIMEI, columns=columns, min_height=100, height_policy= 'min', width=650, autosize_mode='fit_columns')

pECG = figure(title="PM100 reports", plot_width= 1300, plot_height=1840, y_range=(-1,1), x_range=(-1,1))
pECG.image_url('aurl', x=-1, y=1, source=source6, view=viewIMEI, dilate=True)
pECG.xaxis.visible = False
pECG.yaxis.visible = False
...
def selection_change(attrname, old, new):
    ...
    if len(cs) >= 1:
        try:
            ...
            print(f'colour error {e}, with {cs_maxf}')
    else:
...
def newPM100(attr, old, new):
    viewIMEI.filters[0] = GroupFilter(column_name='IMEI', group= new)
    data_tablePDF.visible = False
    data_tablePDF.visible = True

ecgdevices = Select(title="Select by IMEI", value=ecg_devices[0], options=ecg_devices, width=140)
...
gender = Select(title="4. Select your statistics [BC800 only]", value='Woman', options=['Man', 'Woman'], width_policy='min')
...
ecgdevices.on_change('value', newPM100)

gteldevices.on_change('value', newgtel)
gtelslider.on_change('value', gtelperiod)

bpdevices.on_change('value', newbp, bpperiod)
bpslider.on_change('value_throttled', bpperiod)
...
pan1 = Panel(child=layout1, title = 'BP800/D40g, LS802, BM300C')
...
pan2 = Panel(child=layout2, title = 'BC800, BS2001, PS300C')
...
layout5 = column(ecgdevices, data_tablePDF, pECG)
pan5 = Panel(child=layout5, title = 'ECG PM100')

tabs = Tabs(tabs = [pan1, pan2, pan3, pan4, pan5], name = 'g1')
