...
import sys
sys.path.append("..")
#sys.path.append("/M_awsb/sharepoint") # testing only
sys.path.append("/home/bitnami/sharepoint")
from Mapi2 import Mapi, Org, DBconx
from Mdb2 import MonthlyReport_X

# cut here for create data ----------------------------
...
# cut above , -----------------------------------------
r = redis.Redis()
orgs_wk = pickle.loads(r.get("df_M4_orgs_wk"))
dev_org_mon = pickle.loads(r.get("df_M4_dev_org_mon"))

no_orgs = orgs_wk.index.unique('org').to_list()
org_cmap = factor_cmap('xaxis', palette=Spectral11, factors= no_orgs, end=1)
orgIDdict = {k:v[-4:] for k,v in Mapi.getID_Org_id_dict().items()}
orgs = list(orgIDdict.values())#list(Mapi.getOrgs_showas_dict().values())
...
p.add_tools(HoverTool(tooltips = [('Active', '@imei'), ("Month", "@xax{%b}"), ('Dormant', '@dormant')],
...
def callback():
    try:
        ...
        #o1 = Org.getOrgs_showas_dict(True).get(o) # inverted dict but for ID[-4]
        reverse_orgIDdict = {v[-4:]:k for k, v in Mapi.getID_Org_id_dict().items()}
        o1 = reverse_orgIDdict.get(o)
        MonthlyReport_X((o1,),m1, o)
