...
def env_update_save(): # set env, download, assess, save
    ...
    Study.logger(f'Updating Studies {descP34r.info()}')
    ...
    #df2db(db_desc.to_records(index=False, column_dtypes={'Drank_desc' : int, 'Drank_final' : int, 'Drank_out' : int}).tolist(), sql)
        ...
        Study.logger(f'error allocating new studies to db {e}')
    # update contact db
    ...
    # reduce fields
        ...
        Study.logger(f'error in allocating contacts to db {e}')
    ...
    try:
        ...
        Study.logger(f'error in allocating outcomes to db {e}')
